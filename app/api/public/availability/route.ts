...
export async function GET(request: Request) {
    try {
        ...
        if (!serviceOptionId || !dateStr || !slug) {
            return NextResponse.json({ error: 'serviceOptionId, date and slug are required' }, { status: 400 })
        }
        ...
        // 2. Get Settings
        // In the new schema, settings are inside the Tenant model directly
        ...
        if (!targetStaffId) {
            const firstStaff = await prisma.staff.findFirst({ where: { active: true, tenantId: tenant.id } })
